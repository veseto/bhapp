...
foreach ($leagues as $country => $leaguesArray) {
	...
	$sec = time() - $start;
	file_put_contents("/var/www/bh/data_manage/updateStats.txt", "ENGLAND ".date("Y-m-d h:i:s", time())." - seconds: ".$sec."  updated matches: ".$updatedMatches."  added matches to play: ".$updatedMatchesToPlay."\n", FILE_APPEND);
?>
<?php
	...
	$leagues = array('SCOTLAND' => array('PREMIERSHIP', 'CHAMPIONSHIP', 'LEAGUE+ONE', 'LEAGUE+TWO'));
...
foreach ($leagues as $country => $leaguesArray) {
	...
	$sec = time() - $start;
	file_put_contents("/var/www/bh/data_manage/updateStats.txt", "SCOTLAND ".date("Y-m-d h:i:s", time())." - seconds: ".$sec."  updated matches: ".$updatedMatches."  added matches to play: ".$updatedMatchesToPlay."\n", FILE_APPEND);
?>
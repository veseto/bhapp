<?php
	...
	include("../connection.php");
   	...
   	$updatedMatchesToPlay = 0;
				...
				$data1 = file_get_contents("http://www.xscores.com/soccer/Results.jsp?sport=1&countryName=AUSTRALIA&leagueName=A-LEAGUE&leagueName1=A-LEAGUE&seasonName=2013%2F2014&sortBy=R&result=3#.Us6_JvQW3zg");
			...
			for ($j = 0; $j <= $roundCount; $j ++) {
				...
				$round = $j + 1;
			// 	//echo "Round ".$j."<br>";
			 	$data = file_get_contents("http://www.xscores.com/soccer/Results.jsp?sport=1&countryName=AUSTRALIA&leagueName=A-LEAGUE&leagueName1=A-LEAGUE&seasonName=2013%2F2014&sortBy=R&round=$n&result=3#.Us6_JvQW3zg");
							...
							if (($stat === 'Post' || $stat === 'Abd') && $m['state'] === 'Sched') {
								...
								date_default_timezone_set('Europe/Sofia');
							 	if ($ftShort != '-' && $stat==='Fin' && $m['resultShort'] === '-') {
							 		$updatedMatches ++;
									$homeSeries = $mysqli->query("SELECT seriesId FROM series where team='AUSTRALIA' and active=1")->fetch_array()[0];
									if ($ftShort === 'D') {
										 $mysqli->query("update series set active=0, length=length+1 where seriesId='$homeSeries'");
										echo $mysqli->error;
										 $mysqli->query("INSERT INTO series (team, length, active, leagueId) values ('AUSTRALIA', 1, 1, ".$m['leagueId'].")");
										 echo $mysqli->error;
	...
	$res = $mysqli->query("SELECT * from series where active=1 and team='AUSTRALIA'");
	if ($serie = $res->fetch_assoc()) {
		$q1 = "SELECT * FROM `matches` 
				where matches.resultShort='-' and state='Sched' and leagueId=17 order by matchDate asc, matchTime asc limit 1;";
	...
	$sec = time() - $start;
	file_put_contents("/var/www/bh/data_manage/updateStats.txt", "AUSTRALIA ".date("Y-m-d h:i:s", time())." - seconds: ".$sec."  updated matches: ".$updatedMatches."  added matches to play: ".$updatedMatchesToPlay."\n", FILE_APPEND);
?>